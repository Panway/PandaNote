...
typedef NS_ENUM (NSInteger,XDSharePlatform) {
    XDSharePlatformWeixinSession,
    XDSharePlatformWeixinTimeline,
    XDSharePlatformQQSession,
    XDSharePlatformQZone,
    XDSharePlatformWeibo,
    XDSharePlatformClipboard,
    XDSharePlatformImage
};
...
//- (void)shareWithTitle:(NSString *)title content:(NSString *)content url:(NSString *)url platform:(XDSharePlatform)platform;
//- (void)dismissView;
...
@interface XDShareManager : NSObject <WeiboSDKDelegate>
#else
...
/// type: XDSharePlatformQQSession = 2, XDSharePlatformQZone = 3,
/// 分享结果block
@property (nonatomic, copy) void(^shareHandler)(NSString * resultMsg);
@property (nonatomic, copy) void(^loginHandler)(NSDictionary * resultDic);

- (void)shareWithTitle:(NSString *)title content:(NSString *)content url:(NSString *)url image:(id)image platform:(XDSharePlatform)platform completion:(void (^)(NSString * resultMsg))completion;
